...
class HostsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        Task {
            do {
                let tokenPTT = try await Host.getToken()
                //print(tokenPTT!)
                let hostList = try await Host.getHosts(token: tokenPTT!)
                updateUI(with:hostList)
                
            } catch {
                displayError(HostError.HostsNotFound, title: "Error al recuperar los hosts")
            }
        }
    }
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let nextView = segue.destination as! HostsViewController
